...
function migrateDatabase($pdo) {
    ...
    if ($version < 5) {
        ...
        try {
            ...
            $pdo->exec("CREATE TABLE IF NOT EXISTS room_scenes (
                id TEXT PRIMARY KEY,
            ...
            $pdo->exec("CREATE TABLE IF NOT EXISTS room_scene_hotspots (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                scene_id TEXT NOT NULL,
                type TEXT CHECK(type IN ('tra', 'act')),
            ...
            $pdo->exec("CREATE TABLE IF NOT EXISTS room_scene_commands (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                scene_id TEXT NOT NULL,
            ...
            foreach ($roomsList as $r) {
                ...
                $stmtInsertScene = $pdo->prepare("INSERT OR IGNORE INTO room_scenes (id, room_id, is_default, media_id, interactable_desc, dialogue_id, requirements) VALUES (?, ?, 1, ?, NULL, NULL, NULL)");
                $stmtInsertScene->execute([$sceneId, $roomId, $mediaId]);

                // Migrate existing transitions to hotspots
                $stmtTrans = $pdo->prepare("SELECT category, requirements, area FROM room_transitions WHERE room_id = ?");
                $stmtTrans->execute([$roomId]);
                $transList = $stmtTrans->fetchAll(PDO::FETCH_ASSOC);
                foreach ($transList as $t) {
                    $stmtTransLabel = $pdo->prepare("SELECT label FROM transitions WHERE id = ? LIMIT 1");
                    $stmtTransLabel->execute([$t['category']]);
                    $tLabel = $stmtTransLabel->fetchColumn() ?: $t['category'];
                    
                    $stmtInsertHotspot = $pdo->prepare("INSERT INTO room_scene_hotspots (scene_id, type, target_id, label, coords_json, requirements) VALUES (?, 'tra', ?, ?, ?, ?)");
                    $stmtInsertHotspot->execute([$sceneId, $t['category'], $tLabel, $t['area'], $t['requirements']]);
                }

                // Migrate existing interactables to hotspots
                $stmtInts = $pdo->prepare("SELECT interactable_id, requirements FROM room_interactables WHERE room_id = ?");
                $stmtInts->execute([$roomId]);
                $intsList = $stmtInts->fetchAll(PDO::FETCH_ASSOC);
                foreach ($intsList as $i) {
                    $stmtIntLabel = $pdo->prepare("SELECT label FROM interactables WHERE id = ? LIMIT 1");
                    $stmtIntLabel->execute([$i['interactable_id']]);
                    $iLabel = $stmtIntLabel->fetchColumn() ?: $i['interactable_id'];
                    
                    $stmtInsertHotspot = $pdo->prepare("INSERT INTO room_scene_hotspots (scene_id, type, target_id, label, coords_json, requirements) VALUES (?, 'act', ?, ?, NULL, ?)");
                    $stmtInsertHotspot->execute([$sceneId, $i['interactable_id'], $iLabel, $i['requirements']]);
                }
            }
